...
到這裡，第 01 章的 🔰 主軌走完了：容器怎麼蓋（refresh）、圖怎麼畫（BeanDefinition）、工人怎麼加工（BPP）、依賴怎麼裁決、值哪來（Environment）、消息怎麼傳（事件）。但整章刻意迴避了一題：兩顆 singleton **互相** `@Autowired`——先有雞還是先有蛋？容器為什麼（有時）沒炸？答案藏在三層快取裡。

🔬 想深入：[循環依賴與三級快取](deep-circular-dependency.md)
